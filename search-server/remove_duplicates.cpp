...
void RemoveDuplicates (SearchServer& search_server) {
    ...
    std::set <std::string_view> set_from_test_id_map = {};
    std::set <int> duplicates={};
    std::map <std::set<std::string_view>, int> nabor_slov_vseh_documents; // int id, set слова
    for (int testing_id : search_server) {
        
        const std::map <std::string_view, double>& test_id_map = search_server.GetWordFrequencies(testing_id);
